...
const ActiveCell: React.FC<Props> = (props) => {
 ...
 const handleFilesChange = async (
          ...
          const uploadPromises = uniqueFiles?.map(async (file) => {
            try {
              if (file.name.includes('*')) {
                return { file, status: 'existing', data: { name: file.name } };
              }
  ...
  return { file, status: 'memory_full', reason: 'no memory' };
                  ...
                  if (parsedResponse?.id && parsedResponse?.name) {
                    ...
                    return { file, status: 'failed', reason: 'invalid response' };
                  }
                } catch (error) {
    ...
    new File([new Blob()], (file.name?.split('*')[0] ?? 'default'))
      ...
      try {
            ...
            try {
              ...
              if (
                ...
                (typeof response === 'object' &&
                  response !== null &&
                  ('status' in response && (response as any).status === 'SUCCESS' ||
                   'responseCode' in response && (response as any).responseCode === 200))
              ) {
        ...
        if (deletedCount > 0) {
          ...
          setSelectedFiles(
              updatedFileDetails?.map((file) => new File([new Blob()], file.name.split('*')[0]))
          );
          ...
          if (validFiles.length < updatedFileDetails.length) {
           ...
           setSelectedFiles(
                validFiles.map((file) => new File([new Blob()], file.name.split('*')[0]))
            );
...
const handleClick = (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
  const fileElement = (e.target as HTMLElement).closest('.ff-attachment-files');
  const isIconClick = (e.target as HTMLElement).closest('.ff-icon-container, .ff-icon-click, .ff-icon-danger');
    ...
    if (cell?.inputType?.type === 'file' && cell?.value) {
      try {
        const parsedFiles: File[] = JSON.parse(cell.value).map(
          (file: Types.AttachmentApi) => new File([new Blob()], file.name.split('*')[0] ?? 'default')
        );
    ...
    >
      {cell?.inputType?.type === 'dropDown' ? (
        ...
        <textarea
          ...
          style={{ ...cell?.style, textDecoration: 'normal' }} // needed textDecoration
          value={cell?.value}
          disabled={false}
        />
      )}
